...
#include <src/utils/graph/graph.h>
#include <random>
...
void RRT::run() {
    ...
    do {
        ...
        dist = Utils::distance(originPos(), nextPos);

        // Remove points when robot passes throgh it
        if(dist < 2.5f*ROBOT_RADIUS) {
           if(_path.size() > 2)
               _path.removeAt(last-1);
        }

    } while(dist < 2.0f*ROBOT_RADIUS && _path.size() > 2);

    // Path following: analyze segments to get closest segment
    if(_path.size() > 2) {
        float closestSegmentDistance = Utils::distanceToSegment(_path.at(_path.size()-2), _path.at(_path.size()-3), originPos());
        for(int i=_path.size()-3; i>=1; i--) {
            Position pos1 = _path.at(i);
            Position pos2 = _path.at(i-1);

            float dist = Utils::distanceToSegment(pos1, pos2, originPos());
...
bool RRT::obstacleCloseToOrigin(const RRTObstacle &obst, float extraRadius) {
    float distObstOrigin = Utils::distance(originPos(), obst.getPos());
...
void RRT::checkGoalInsideObstacle() {
    for(int i=_obstacleList.size()-1; i>=0; i--) {
        ...
        float distObstGoal = Utils::distance(goalPos(), obst.getPos());
...
*/
    ...
    return Position(true, xRand, yRand);
}
...
bool RRT::canConnect(const Position &posA, const Position &posB, float extraRadius, float extraRadiusIfClose) {
    ...
    for(int i=0; i<_obstacleList.size(); i++) {
        ...
        // Check can connect
        float dist = Utils::distanceToSegment(posA, posB, obst.getPos());
        if(dist < obstRadius)
            return false;
    }
...
Position RRT::getVector(const Position &near, const Position &rand, float vectorLength) {
    float theta = Utils::getAngle(near, rand);
    return Position(true, vectorLength*cos(theta), vectorLength*sin(theta));
}
...
void RRT::minimizePath(int start, int end) {
    ...
    for(int i=start; i<=end; i++) {
        ...
        if(i>start) {
            ...
            graph.connect(vertex, last, Utils::distance(vertex->getPosition(), last->getPosition()));
        }
    ...
    for(int i=0; i<vertices.size(); i++) {
        ...
        for(int j=0; j<vertices.size(); j++) {
            ...
            if(canConnect(pos1, pos2, ROBOT_RADIUS, ROBOT_RADIUS)) {
                float weight = Utils::distance(pos1, pos2);
...
void RRT::smoothPath() {
    ...
    if(_path.size()==2 && Utils::distance(_path.first(), _path.last()) > 2*_smoothPathResolution) {

        // Calc num points to add
        int numPoints = Utils::distance(_path.first(), _path.last())/_smoothPathResolution;
        Position vec = getVector(_path.first(), _path.last(), _smoothPathResolution);

        // Add points
        for(int i=0; i<numPoints; i++) {
            Position lastPos = _path.at(_path.size()-2);
            Position newPos(true, lastPos.x() + vec.x(), lastPos.y() + vec.y());
    ...
    while(keepRunning) {
        ...
        for(int i=1; i<_path.size()-1; i++) {
            float distA = Utils::distance(_path.at(i), _path.at(i-1));
            float distB = Utils::distance(_path.at(i), _path.at(i+1));
            ...
            if(distA > _smoothPathResolution) {
                ...
                Position newPoint(true, _path.at(i).x() + vec.x(), _path.at(i).y() + vec.y());
...
QList<Position> RRT::findPath(const Position &origin, const Position &goal) {
    ...
    while(foundPath==false) {
        ...
        Position newPos(true, nearNode->getPosition().x() + unitVec.x(), nearNode->getPosition().y() + unitVec.y());
...
float RRT::getDistance() const {
    ...
    for(int i=0; i<_path.size()-1; i++)
        dist += Utils::distance(_path.at(i), _path.at(i+1));
    return dist;
}
