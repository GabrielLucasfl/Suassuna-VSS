...
void RobotFrame::reset() {
    setRobotData(Colors::Color::BLUE, 0, DEFAULT_PATTERN_SIZE);
...
void RobotFrame::setRobotData(Colors::Color teamColor, quint8 robotId, int patternSize) {
    ...
    QString path = QString(":/resources/robots/%1/%2.png")
            .arg((teamColor == Colors::Color::YELLOW ? "yellow" : "blue"))
            .arg((teamColor == Colors::Color::YELLOW ? QString("y%1").arg(robotId) : QString("b%1").arg(robotId)));
    QPixmap pixmap(path);

    // Check if loaded pattern is null
    if(pixmap.isNull()) {
        std::cout << Text::red("[ERROR] ", true) + Text::bold("[setRobotData] Failed to load QPixMap at RobotFrame") + '\n';
        return ;
    }
...
void RobotFrame::setRobotRadioStatus(bool status) {
    ...
    // Check if loaded pattern is null
    if(pixmap.isNull()) {
        std::cout << Text::red("[ERROR] ", true) + Text::bold("[setRobotRadioStatus] Failed to load QPixMap at RobotFrame") + '\n';
        return ;
    }
